    ...
    <div class="mini-player" v-show="!fullScreen" @click="showNormalPlayer">
      ...
      <div class="control">
        <progress-circle :radius="32" :progress="progress">
          <!-- 此处@click需要stop,因为不能让点击事件冒泡影响到父级点击事件showNormalPlayer -->
          <i
            class="icon-mini"
            :class="miniPlayIcon"
...
export default {
  ...
  setup() {
    const playlistRef = ref(null); // 通过playlistRef获取到组件实例
...
.mini-player {
  ...
  &.mini-enter-active,
  &.mini-leave-active {
    // 贝塞尔动画曲线cubic-bezier(x1, y1, x2, y2)
