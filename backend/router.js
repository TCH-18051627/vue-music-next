 ...
 * 后端之间的通讯不存在跨域限制
 * 即前端请求 -> router.js后端请求 -> 第三方接口接收
 * 第三方接口返回 -> router.js后端返回 -> 前端接收
 */
...
function registerRecommend(app) {
  app.get("/api/getRecommend", (req, res) => {
    ...
    get(url, {
      ...
      // 判断响应结果data的code是否是响应成功错误码ERR_OK(0)
      if (data.code === ERR_OK) {
        // 处理轮播图数据
        ...
        for (let i = 0; i < len; i++) {
          ...
          const sliderItem = {};
          // 单个轮播图数据包括 id(用于区分数据)、pic(图片)、link(点击图片跳转的URL) 等字段
          sliderItem.id = item.id;
          sliderItem.pic = item.cover;
          if (jumpPrefixMap[item.jumptype]) {
            // link跳转链接需要根据内部字段做逻辑处理
            sliderItem.link =
    ...
    get(url, {
      ...
      if (data.code === ERR_OK) {
        ...
        singerList.forEach(item => {
          ...
          if (key) {
            // 如果对象歌手Map中没有这个key,那么就设置一个空的key的键
            if (!singerMap[key]) {
              singerMap[key] = {
                title: key,
                list: []
            ...
            // 每个字母下面会有多名歌手
            // item是单条数据，通过map处理[item]后再取第一条
            singerMap[key].list.push(map([item])[0]);
            // list = [{id, mid, name, pic}, {...}, {...}]
          }
        });
      ...
      return post(url, data).then(response => {
        ...
        if (data.code === ERR_OK) {
          ...
          midInfo.forEach(info => {
            // 获取歌曲的真实播放 URL(即domian + info.purl)
  ...
  app.get("/api/getTopList", (req, res) => {
    ...
    get(url, {
      ...
      if (data.code === ERR_OK) {
        ...
        // 遍历group取每一项item，再对item中的toplist(排行榜列表)部分遍历
        // 取每一项listItem(排行榜单项模块)，将相关数据作对象属性的封装，再push到topList中
        // 此时topList中每一项对应一个模块的排行榜数据
        group.forEach(item => {
...
function registerTopDetail(app) {
  app.get("/api/getTopDetail", (req, res) => {
    const url = "https://u.y.qq.com/cgi-bin/musics.fcg";
    // 请求需要传入query: {id, period} 供第三方服务需要
...
function registerSearch(app) {
  app.get("/api/search", (req, res) => {
    ...
    // 请求时包含的参数
    // query: 查询的字符串
    // page: 页码 (分页请求,需要上拉加载的效果)
    // showSinger: 查询结果是否需要包含歌手数据 true/false
    ...
    get(url, data).then(response => {
      ...
      if (data.code === ERR_OK) {
        ...
        // 根据返回的songData获取当前这页返回的数据量、页码、最多能获取到的总数据量
        const { curnum, curpage, totalnum } = songData;
        // hasMore为true表示能继续上拉加载新一页搜索结果，false表示加载完了
